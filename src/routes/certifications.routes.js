...
async function retrieveAll(req, res, next) {
    try {
        let certifications = await certificationRepository.retrieveAll();
...
async function approveCertification(req, res, next) {
    try {
        const { id } = req.params;
        const certification = await certificationRepository.approveCertification(id);
        if (!certification) {
...
async function rejectCertification(req, res, next) {
    try {
        const { id } = req.params;
        const certification = await certificationRepository.rejectCertification(id);
        if (!certification) {
            return res.status(404).json({ message: "Demande de certification non trouvée" });
        }
        res.status(200).json(certification.toJSON());
